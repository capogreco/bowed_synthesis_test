  ...
  process(inputs, outputs, parameters) {
    ...
    for (let i = 0; i < outputChannel.length; i++) {
      ...
      if (this.bowEnvelope > 0.001) {
        ...
        const friction = (Math.random() - 0.5) * 0.5; // Reduced amplitude noise
        
        // Mix saw with friction noise (no pulse wave needed)
        const toneSignal = sawWave * sawPulseMix + friction * (1.0 - sawPulseMix);
        
        // Add noise
        const noiseSignal = Math.random() * 2.0 - 1.0;
        
        // Mix tone and noise with dynamic pressure-based roughness
        const dynamicNoiseMix = Math.max(0, Math.min(1, toneNoiseMix - pressureRoughness));
      ...
      // Apply LPF with dynamic cutoff based on bow pressure
      // Recalculate LPF coefficients if needed for pressure-based brightness
      if (this.bowEnvelope > 0.001 && i === 0) { // Only update once per buffer for efficiency
        const baseCutoff = this._cachedLpfCutoff;
        const dynamicCutoff = Math.min(1.0, baseCutoff * (1.0 + pressureBrightness * 0.5));
