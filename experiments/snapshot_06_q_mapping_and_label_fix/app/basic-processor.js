// basic-processor.js (ContinuousExcitationProcessor with resonant LPF and aggressive Q mapping)
class ContinuousExcitationProcessor extends AudioWorkletProcessor {
  ...
  _calculateFilterCoefficients() {
    ...
    // *** Aggressive Q Mapping with Math.pow(resonanceParam, 3) and low maxQ ***
    const minQ = 0.707; 
    const maxQ = 2.5;  // VERY conservative max Q.
    let actualQ;

    const currentResonanceParam = Math.max(0.0, Math.min(1.0, this.resonanceParam));
    // Apply a higher power to the slider value to make the low end extremely fine-grained
    const mappedResonanceParam = Math.pow(currentResonanceParam, 3); // Cube the slider value

    if (mappedResonanceParam <= 0) { 
        actualQ = minQ;
    } else if (minQ <= 0) { // Defensive case
        actualQ = mappedResonanceParam * maxQ; 
    } else {
        actualQ = minQ * Math.pow(maxQ / minQ, mappedResonanceParam);
    }
    // Ensure Q is at least minQ and no more than maxQ (or slightly above due to potential pow overshoot)
    actualQ = Math.max(minQ, Math.min(actualQ, maxQ * 1.01)); // Clamp strictly to minQ and maxQ
    // *** End of Aggressive Q Mapping ***
    ...
    // console.log(`[Processor] Filter Coeffs: F0=${F0.toFixed(1)}, Q=${Q.toFixed(1)} (ResoParam: ${this.resonanceParam.toFixed(2)}, MappedReso: ${mappedResonanceParam.toFixed(2)})`);
  }
