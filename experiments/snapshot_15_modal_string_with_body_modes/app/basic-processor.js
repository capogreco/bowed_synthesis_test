...
class ContinuousExcitationProcessor extends AudioWorkletProcessor {
  ...
  _handleMessage(event) {
    ...
    if (data.type === 'pluckString') {
      ...
      this._resetStringModeStates(); // Renamed from _resetModalStates
      this._resetLpfState();
  ...
  process(inputs, outputs, parameters) {
    ...
    // Handle excitation pulse
    let excitationSamples = 64; // Longer excitation burst
    let excitationApplied = false;
    
    for (let i = 0; i < outputChannel.length; i++) {
      // Apply excitation as a burst with exponential decay
      let currentInputToFilters = 0.0;
      if (this.applyExcitation && i < excitationSamples) {
        // Exponential decay envelope
        const decay = Math.exp(-5.0 * i / excitationSamples);
        currentInputToFilters = excitationLevel * decay;
        excitationApplied = true;
      }
    ...
    // Reset excitation flag after processing the buffer
    if (excitationApplied && this.applyExcitation) {
      this.applyExcitation = false;
    }
